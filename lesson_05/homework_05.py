...
min_year, min_volume, max_price = search_criteria

# task 1. Фільтрація
filtered_cars = [
    (brand, info)
    for brand, info in car_data.items()
        if info[1] >= min_year and info[2] >= min_volume and info[4] <= max_price
]
print(f"# task 1 (filtered). {filtered_cars}")

# task 2. Сортування за ціною за зростанням
sorted_cars = sorted(filtered_cars, key=lambda item: item[1][4])
print(f"# task 2 (sorted). {sorted_cars}")

# task 3. Вивід перших 5 знайдених авто
print("# task 3 (the first 5):")
for brand, info in sorted_cars[:5]:
    print(f"{brand}: {info}")